...
pub async fn user_connected(ws: WebSocket, connections: Connections, sessions: Sessions) {
    ...
    connections_reader.iter()
        .for_each(|(_user_id, connect)| {
            connect.send(Message::Binary(disconnected.clone())).expect("cannot send message");
        });
...
async fn user_message(
    ...
    match request {
        ...
        SignalMessage::Disconnected(user_id) => {
            error!("dixconet user message {}", user_id);
        }
    }
    Ok(())
}
